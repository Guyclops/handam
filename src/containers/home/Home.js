...
const Home = ({
  ...
  const navigateAction = useCallback(title => {
    switch (title) {
      ...
      case values.homeMenuTitle.MAP:
        navigateSchoolMap();
        break;
      case values.homeMenuTitle.PHONEBOOK:
        navigatePhoneBook();
    }
  }, []);
  ...
  const navigatePhoneBook = useCallback(async () => {
    await PhoneBookActions.initMainPage();
    await PhoneBookActions.initMainTotal();
    await PhoneBookActions.hansungOfficeList(contactOffice.length/12, 12);
    await PhoneBookActions.professorList(contactProfessor.length/6, 6);
    navigation.navigate("phoneBookStack");
  }, []);
  ...
  const initCall = useCallback(async () => {
    ...
    await Promise.all([
      ...
      AlarmActions.alarmIsNonSubjectAction(
        user.isNonSubjectPointAlarm == 1 ? true : false
      ),
...
export default connect(({ home, alarm, hansung, signin, phonebook }) => ({
  ...
  user: signin.user,
  contactProfessor: phonebook.contactProfessor,
  contactOffice: phonebook.contactOffice
}))(Home);
